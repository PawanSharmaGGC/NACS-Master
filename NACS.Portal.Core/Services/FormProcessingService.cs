...
public class FormProcessingService : IFormProcessingService
{
    ...
    public async Task<ProcessingResult> ProcessFormDataAsync<T>(
        T formData,
        string formKey,
        ...
        bool hasRecaptcha = false) where T : class
    {
        if (formData == null)
        {
            ...
            return new ProcessingResult { success = false, message = "Invalid form data." };
        }
        ...
        try
        {
            if (hasRecaptcha)
            {
                ...
                if (!await VerifyRecaptcha(recaptchaResponse))
                    return new ProcessingResult { success = false, message = "reCaptcha validation failed." };
            }
        ...
        catch (Exception ex)
        {
            ...
            return new ProcessingResult { success = false, message = "The entered values cannot be saved. Please see the fields below for details." };
        }
    }

    private async Task<ProcessingResult> ProcessFormDataInternal<T>(
        T formData,
        string formKey,
        string emailSettingsKey,
    ...
    {
        var logService = CMS.Core.Service.Resolve<IEventLogService>();
        logService.LogException(nameof(FormProcessingService), methodName, ex, additionalInfo);
    }

...
namespace NACSMagazine.PageTemplates.CategoryPage
{
    public class CategoryPageTemplateController : Controller
    {
        ...
        public async Task<ActionResult> Index()
        {
            ...
            if (success)
            {
                articles = await GetCategoryArticlesAsync(string.Empty, value, 9);
            }
            else
            {
                articles = await GetCategoryArticlesAsync(string.Empty, 1, 9);
            }

            page.ArticleList = await SetTaggedArticlesListAsync(page, articles, page.PageNumber, page.PageSize);
        ...
        public async Task<PagedList<Article>> GetCategoryArticlesAsync(string year, int pageNumber, int pageSize)
        {
        ...
        public async Task<IActionResult> IndexAsync(NACSMagazine.CategoryPage page)
        {
            ...
            var articles = await GetCategoryArticlesAsync(page.SelectedYear, page.PageNumber, page.PageSize);
        ...
        public async Task<PagedList<Article>> SetTaggedArticlesListAsync(NACSMagazine.CategoryPage page, List<Article> articles, int pageNumber, int pageSize)
        {
            var articleList = new PagedList<Article>();

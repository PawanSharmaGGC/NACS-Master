//--------------------------------------------------------------------------------------------------
// <auto-generated>
//
//     This code was generated by code generator tool.
//
//     To customize the code use your own partial class. For more info about how to use and customize
//     the generated code see the documentation at https://docs.xperience.io/.
//
// </auto-generated>
//--------------------------------------------------------------------------------------------------

using System;
using System.Collections.Generic;
using CMS.ContentEngine;
using CMS.Websites;

using NACSMagazine.PageTemplates.MagazineArticlePage;

namespace NACSMagazine
{
	/// <summary>
	/// Represents a page of type <see cref="ArticlePage"/>.
	/// </summary>
	[RegisterContentTypeMapping(CONTENT_TYPE_NAME)]
	public partial class ArticlePage : IWebPageFieldsSource
	{
		/// <summary>
		/// Code name of the content type.
		/// </summary>
		public const string CONTENT_TYPE_NAME = "NACSMagazine.ArticlePage";


		/// <summary>
		/// Represents system properties for a web page item.
		/// </summary>
		[SystemField]
		public WebPageFields SystemFields { get; set; }


		/// <summary>
		/// ArticleContent.
		/// </summary>
		public IEnumerable<Article> ArticleContent { get; set; }


		/// <summary>
		/// EditorsPicks.
		/// </summary>
		public IEnumerable<Article> EditorsPicks { get; set; }


		/// <summary>
		/// YouMayAlsoLikeArticles.
		/// </summary>
		public IEnumerable<Article> YouMayAlsoLikeArticles { get; set; }

		public ArticleListWidgetViewModel ArticleListWidget { get; set; }

	}
}